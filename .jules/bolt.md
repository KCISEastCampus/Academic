...
## 2026-09-20 - DOM Querying in Keydown Events
**Learning:** Discovered an anti-pattern in `assets/js/search.js` where `querySelectorAll` was executed on every `keydown` event (e.g., arrow keys) to fetch search result items. Querying the DOM is an expensive $O(N)$ synchronous operation and running it repeatedly inside high-frequency user interactions can lead to input lag and frame drops.
**Action:** Always cache dynamically generated DOM elements (like rendered search result items) into an array when they are created, and use that cache during high-frequency events like keyboard navigation instead of repeatedly querying the DOM.
