...
function initImageModal() {
  ...
  const modalHTML = `
    <div id="imageModalOverlay" class="image-modal-overlay" hidden aria-hidden="true">
      ...
      <div class="image-modal-content">
        <img id="imageModalImg" alt="放大图片" decoding="async">
      </div>
    </div>
  `;
  ...
  images.forEach((img) => {
    if (
      img.dataset.imageModalBound === "true" ||
      img.closest("#imageModalOverlay") ||
      !img.src
    ) {
      return;
    }
    ...
    img.addEventListener("click", function (e) {
      e.stopPropagation();
      openModal(this.currentSrc || this.src, this.alt);
    });
  });
