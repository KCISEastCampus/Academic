...
> **Exam info**: OxfordAQA 9202 · Linear qualification · 2 papers (1h 30m each, 90 marks each) · 9-point grading (1–9)
>
> **Assessment Objectives**: AO1 Knowledge 30% · AO2 Application 40% · AO3 Data analysis 20% · AO4 Scientific procedures 10%
...
|---|---|---|
| **Solid** | fixed, regular | vibrate about fixed positions |
| **Liquid** | close, irregular | move around each other |
| **Gas** | far apart | fast, random motion |

**Inter-conversions**: melting, boiling/evaporation (solid→liquid→gas, energy absorbed); condensing, freezing (energy released).
...
- An atom has a small central nucleus (protons + neutrons) with electrons around it.
...
3. What does the atomic number tell you? **(Number of protons)**
...
- Ionic compounds form a **giant lattice** held by strong electrostatic forces of attraction.
- **Example**: sodium chloride (NaCl). Group 1 metals form +1 ions; Group 7 halogens form −1 ions.

**Covalent bonding** (non-metals): atoms **share** pairs of electrons → strong bonds.

- Simple molecules: H₂, Cl₂, O₂, N₂, HCl, H₂O, NH₃, CH₄.
...
|---|---|---|
| **Ionic compounds** | high (strong bonds in lattice) | conduct when melted/dissolved (ions free to move) |
| **Simple molecules** | low (weak intermolecular forces) | do not conduct (no overall charge) |
...
3. Why do simple molecules have low boiling points? **(Weak intermolecular forces)**
...
- **Displacement reactions** establish positions in the reactivity series.
- Metals **less reactive than carbon** are extracted by **reduction with carbon** (e.g. iron in the blast furnace).
- Metals **more reactive than carbon** (e.g. aluminium) extracted by **electrolysis** of molten compounds.
- **Phytomining** (plants absorb metal compounds) and **bioleaching** (bacteria produce leachate) extract copper from low-grade ores.
- **Recycling** metals saves limited resources, energy, and reduces environmental impact.
...
- Half equations, e.g. at the anode: $$2Cl^- \rightarrow Cl_2 + 2e^-$$
- Used for **electroplating** (copper, silver plating).
- **Aluminium**: electrolysis of molten aluminium oxide + **cryolite** (lowers melting point). Carbon anode reacts with oxygen → CO₂.
- **Electrolysis of sodium chloride solution** → hydrogen + chlorine + sodium hydroxide (important industrial reagents).
...
3. What is an alloy? **(A mixture of elements, at least one a metal)**
...
**Precipitates with NaOH**: aluminium/calcium/magnesium → white (only aluminium hydroxide dissolves in excess); copper(II) → blue; iron(II) → green; iron(III) → brown.

**Other ion tests**: carbonates → CO₂ (limewater cloudy); halides with silver nitrate → chloride white, bromide cream, iodide yellow; sulfates with barium chloride → white precipitate.
...
3. What test identifies a sulfate? **(White precipitate with barium chloride)**
...
- **pH scale** (0–14): pH 7 is neutral. H⁺(aq) makes solutions acidic; OH⁻(aq) makes them alkaline.
...
- **Titration**: volumes of acid and alkali that react are measured using an indicator; concentration of one reactant can be found if the other is known.
...
3. What is the molar gas volume at RTP? **(24 dm³)**
...
- **Further down the group → more reactive** (outer electron more easily lost).

**Group 7 (halogens)**:
- React with metals to form −1 ionic compounds.
...
3. Name a property of transition metals. **(Form coloured compounds / act as catalysts)**
...
- Reactions occur only when particles **collide** with sufficient energy (≥ **activation energy**).

| Factor | Effect on rate | Reason |
|---|---|---|
| **Temperature ↑** | increases | particles faster → more frequent, more energetic collisions |
| **Pressure ↑** (gases) | increases | more frequent collisions |
| **Concentration ↑** | increases | more frequent collisions |
| **Surface area ↑** | increases | more frequent collisions |

- **Catalysts** speed up reactions but are **not used up**. Different reactions need different catalysts.
...
| Change | Endothermic yield | Exothermic yield |
|---|---|---|
...
1. Sulfur + oxygen → sulfur dioxide: $$S + O_2 \rightarrow SO_2$$
...
3. What catalyst is used in the Haber process? **(Iron)**
...
|---|---|---|
| **Exothermic** | transfers energy to surroundings (ΔH negative) | combustion, oxidation, neutralisation |
| **Endothermic** | takes in energy from surroundings (ΔH positive) | thermal decomposition |
...
- During a reaction: energy is **supplied to break bonds**, energy is **released when bonds form**.
- **Exothermic**: energy released from forming new bonds > energy needed to break existing bonds.
...
3. What do catalysts do to activation energy? **(Lower it)**
...
- **Alkanes** (saturated): general formula **CₙH₂ₙ₊₂** (all single C–C bonds). E.g. methane, ethane, propane.
- Properties of hydrocarbons depend on molecular size: boiling point, viscosity, flammability.
- Combustion products: CO₂, water vapour, CO, SO₂, oxides of nitrogen, particulates. SO₂ and NOₓ cause **acid rain**; CO₂ → climate change; particulates → global dimming.
...
- **Polymerisation**: many small molecules (**monomers**) join to form very large molecules (**polymers**) — e.g. ethene → poly(ethene).
...
- **Esters** (functional group –COO–): e.g. **ethyl ethanoate** (ethanol + ethanoic acid). Volatile compounds with distinctive smells — used as **flavourings and perfumes**.
...
3. What test identifies an alkene? **(Turns bromine water from orange to colourless)**
...
| Equation | Formula |
|---|---|
| Concentration | $$\text{Concentration} = \frac{\text{moles}}{\text{volume (dm}^3)}$$ |
| Moles | $$\text{moles} = \frac{\text{mass}}{\text{molar mass}}$$ |
| Rate of reaction | $$\frac{\text{amount}}{\text{time}}$$ |
| Energy change | $$Q = mc\Delta T$$ |
...
*OxfordAQA International GCSE Chemistry (9202). Latest spec: [oxfordaqa.com/9202](https://oxfordaqa.com/9202).*
